...
exports.registerUser = async (req, res) => {
    try {
        const { name, email, password, nickname, about, occupation, hometown, website } = req.body;
...
exports.followUser = async (req, res) => {
    try {
        ...
        if (currentUser.following.includes(req.params.id)) {
            return res.status(400).json({ message: 'Du följer redan denna användare'});
        }
...
exports.unfollowUser = async (req, res) => {
    try {
        if (req.user._id.toString() === req.params.id) {
            return res.status(400).json({ message: 'Du kan inte sluta följa dig själv' });
        }
        ...
        if (!currentUser.following.includes(req.params.id)) {
            return res.status(400).json({ message: 'Du följer inte denna användare' });
        }
